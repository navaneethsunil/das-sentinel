...
- [~] **Log retention & integrity** — code done: structured JSON stdout logs, append-only audit + no-mutate `das_app` role, and off-box immutable audit archival to the WORM store (`scripts/archive_audit_log.py`, `AUDIT_ARCHIVE_RETENTION_DAYS`). Remaining is deployment: off-box log-shipping collector, the retention **value**/schedule, and host NTP — see `security/log-retention-runbook.md`.
