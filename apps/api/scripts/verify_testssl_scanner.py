"""M4 live proof: the testssl.sh adapter runs a real TLS assessment end-to-end.

Runs INSIDE the `scanners` image (real testssl.sh 3.2.4) against real Postgres +
MinIO, assessing the `tls-target` service (sandbox/tls_target.Caddyfile — a Caddy
HTTPS endpoint with an internal-CA/untrusted cert, internal network only). Drives
a scanner scan the way a launched scan does: launch_scan freezes an envelope
naming the `testssl` scanner, orchestrate_scan re-derives + runs it through the
real InProcessOwner → run_scanners → a killable SubprocessOwner running the actual
`testssl.sh -p -S -h --jsonfile <file> https://tls-target:443` → raw capture →
normalize → TLS findings.

Proves:
  1. a real testssl.sh assessment finalizes COMPLETED via the framework;
  2. the scanner_run records the real testssl version, the recon-tls config, and
     the raw-evidence pointer;
  3. testssl flagged the untrusted internal-CA cert (and any protocol/header
     issues) → automated/open findings carrying scanner_run_id + testssl check
     rule ids, citing the raw evidence (testssl's own JSON, hash-verified);
...
TARGET_URL = "https://tls-target:443"
failures: list[str] = []
...
async def _run(sm, store, *, org_id, user_id) -> None:  # noqa: ANN001, PLR0915
    ...
    async with sm() as s:
        ...
        sr = runs[0]
        check("scanner is testssl", sr.scanner_name == "testssl")
        check(
            "real testssl version captured",
            bool(sr.scanner_version) and sr.scanner_version != "unknown",
        )
        check("scanner_run COMPLETED", sr.status is ScanStatus.COMPLETED)
        check(
            "config records the TLS recon mode",
            sr.config.get("mode") == "recon-tls" and sr.config.get("intrusive") is False,
        )
        ...
        check(
            "raw evidence is a testssl JSON array with severities",
            isinstance(parsed, list)
            and any(isinstance(e, dict) and "severity" in e for e in parsed),
        )
        ...
        check("testssl produced TLS findings (>=1 finding)", len(findings) >= 1)
        check(
        ...
        check(
            "findings carry testssl check rule ids + severity",
            all(f.rule_id and f.severity is not None for f in findings),
        )
        check(
            "the untrusted internal-CA cert is flagged (>=1 non-informational)",
            any(f.severity is not Severity.INFORMATIONAL for f in findings),
        )
        cited_ok = True
        for f in findings:
            links = (
...
async def main() -> int:
    ...
    async with sm() as s:
        org = Organization(name="verify-testssl-org")
        s.add(org)
        await s.flush()
        org_id = org.id
        ...
        user = User(
            ...
            email="verify-testssl@example.com",
            password_hash=pw.hash("verify-testssl-throwaway"),
            display_name="Verify Testssl",
