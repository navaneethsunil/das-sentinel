"""M4 live proof: the Nuclei adapter runs a real active scan end-to-end.

Runs INSIDE the `scanners` image (real nuclei 3.11.0) against real Postgres +
MinIO, scanning the deliberately-insecure `vuln-target` service (sandbox/
vuln_web.py, internal network only) with the VENDORED safe-active template bundle.
Drives a scanner scan the way a launched scan does: launch_scan freezes an
envelope naming the `nuclei` scanner, orchestrate_scan re-derives + runs it
through the real InProcessOwner → run_scanners → a killable SubprocessOwner
running the actual `nuclei -target <url> -jsonl -templates <bundle>` → raw capture
→ normalize → findings.

Proves:
  1. a real nuclei scan finalizes COMPLETED via the framework;
  2. the scanner_run records the real nuclei version + the content digest of the
     vendored template bundle (rules_digest) + safe-active config (interactsh off,
     intrusive tags excluded) + the raw-evidence pointer;
  3. nuclei fired the bundled templates on the header-less target → automated/open
     findings carrying scanner_run_id + template rule ids + url location, citing
     the raw evidence (nuclei's own JSONL, hash-verified);
...
Run:
  ...
  docker compose --profile scanners run --rm --no-deps \
    ...
    -c "cd /app && PYTHONPATH=/app python scripts/verify_nuclei_scanner.py"
"""
...
async def _run(sm, store, *, org_id, user_id) -> None:  # noqa: ANN001, PLR0915
    ...
    async with sm() as s:
        ...
        sr = runs[0]
        check("scanner is nuclei", sr.scanner_name == "nuclei")
        check(
            "real nuclei version captured",
            bool(sr.scanner_version) and sr.scanner_version != "unknown",
        )
        check("scanner_run COMPLETED", sr.status is ScanStatus.COMPLETED)
        check("rules_digest (vendored bundle content hash) recorded", bool(sr.rules_digest))
        check(
            "config records safe-active profile (no interactsh, tags excluded)",
            sr.config.get("profile") == "safe-active"
            and sr.config.get("interactsh") is False
            and "intrusive" in str(sr.config.get("excluded_tags")),
        )
        check("raw_evidence_id set", sr.raw_evidence_id is not None)

        raw = (await load_evidence(s, store, sr.raw_evidence_id)).decode()
        check(
            "raw evidence is nuclei JSONL (>=1 line with template-id)",
            any("template-id" in line for line in raw.splitlines() if line.strip()),
        )
        ...
        check("nuclei fired the bundled templates (>=1 finding)", len(findings) >= 1)
        check(
        ...
        check(
            "findings carry template rule ids + url location",
            all((f.rule_id or "").startswith("das-") and f.location.get("url") for f in findings),
        )
        cited_ok = True
        for f in findings:
            links = (
...
async def main() -> int:
    ...
    async with sm() as s:
        org = Organization(name="verify-nuclei-org")
        s.add(org)
        await s.flush()
        org_id = org.id
        ...
        user = User(
            ...
            email="verify-nuclei@example.com",
            password_hash=pw.hash("verify-nuclei-throwaway"),
            display_name="Verify Nuclei",
