...
def test_kind_for_config_agent() -> None:
    # agent_permission routes to its own kind (different engine than the PyRIT suites).
    config = {"kind": "safe_active_scan", "suites": ["agent_permission"]}
    assert kind_for_config(config) == AGENT_KIND


def test_queue_mapping_matches_worker_images() -> None:
    # LLM suites need PyRIT (redteam image); scanners need semgrep/ZAP; the agent
    # harness is pure Python → the base worker's default queue.
    assert QUEUE_FOR_KIND[SUITE_KIND] == "redteam"
    assert QUEUE_FOR_KIND[SCANNER_KIND] == "scanners"
    assert QUEUE_FOR_KIND[AGENT_KIND] == "celery"
...
async def test_load_scan_kind_agent() -> None:
    session = _FakeSession({"suites": ["agent_permission"]})
    assert await load_scan_kind(session, uuid.uuid4()) == AGENT_KIND  # type: ignore[arg-type]
...
def test_build_owner_for_kind_returns_inprocess_owner() -> None:
    ...
    for kind in (SUITE_KIND, SCANNER_KIND, AGENT_KIND):
