...
> **M3-SEC3 implementation note (2026-07-24):** SAST + SCA already block on High (M1-SEC5); DAST (ZAP baseline against our own running app) was raised to **block on FAIL/High** alerts (WARN/Medium stay report-only) — our app currently reports 0 FAIL. The Semgrep adapter now dogfoods our own scan output in CI (`scripts/dogfood_semgrep.py`). **Container scanning (Trivy image/config) stays report-only.**

> **Trivy HIGH triage (2026-07-28, go-live gate).** Re-ran Trivy HIGH,CRITICAL against freshly-pulled bases. Two buckets:
> - **Fixable — RESOLVED.** The only fixable High/Critical were `tar`/`undici`/`brace-expansion` — traced (by `PkgPath`) to the **base image's global npm** at `/usr/local/lib/node_modules/npm/...`, *not* our app deps (the earlier "our package-lock is patched" note was inaccurate; our Next standalone runtime ships no `node_modules` at all, and the latest `node:24-slim` still bundles the vulnerable npm). Fix: the web runtime stage runs only `node server.js` + a `node -e` healthcheck, so it **removes the bundled global npm/corepack** (`apps/web/Dockerfile` run stage). Re-scan → **0 fixable node-pkg findings**; e2e green (npm was never invoked at runtime). Bonus: no package manager in a prod image = smaller attack surface.
> - **No-fix base OS — reduced (web) / deferred (api).** These were Debian base-layer CVEs with no upstream fix.
>   - **web → distroless.** Moved the web runtime to `gcr.io/distroless/nodejs24-debian12:nonroot` (digest-pinned). The runtime is only `node server.js` (Next standalone, no shell/pkg-manager needed), so distroless cut HIGH/CRITICAL **22 → 6** and dropped the shell + package manager entirely (major attack-surface reduction; runs as nonroot uid 65532). The 6 remaining are all `libssl3` with an upstream fix pending Google's next distroless rebuild — report-only, re-verify when distroless rebuilds. Stayed on **debian12** (not trixie) to match the bookworm build base's native ABI (`sharp` is a dep; the app uses no `next/image` so it's never loaded, but the ABI match keeps it safe). Compose web healthcheck now calls node absolutely (`/nodejs/bin/node`) since distroless has no PATH `node`/shell.
>   - **api → deferred.** The api image can't go distroless — its workers shell out to launch scanner subprocesses in per-run sandboxes (§6a), so it needs a shell + execution environment. Its ~23 HIGH/CRIT are no-fix trixie base CVEs; a minimal/hardened base is a Hardening-gate item.
>   - Digest-pinning `FROM` (TR-25) is **done**. SBOM signing/SLSA is **done** (keyless Sigstore attestation, above). Container-blocking stays deferred rather than blocking on an upstream base bump. Re-verify at the Hardening gate.

> **ASVS 5.0 app-wide L2 review (2026-07-28, Hardening gate).** Completed the app-wide **Level 2** self-assessment across the chapters beyond the M1 L3 subsystems — [`security/ASVS-L2-review-hardening.md`](security/ASVS-L2-review-hardening.md). Verdict: **meets L2 app-wide** (L3 for crypto; auth/session/authz/audit already L3 in the M1 review). Backed by four independent read-only code-evidence sweeps (injection/exec, SSRF/egress/TLS, crypto/files/config, web/API/error). The security-defining surfaces (command execution — exec-form, no `shell=True`; SSRF/egress — deny-wins + metadata-IP block + fail-closed; file upload — zip-slip/zip-bomb/symlink defended; crypto — hash-on-read, Argon2id; secrets — refs-only, SecretStr) are all Met. Output is a prioritized remediation backlog **SEC-DEBT-6…15**; highest-value next fixes are **SEC-DEBT-6** (DNS-rebinding TOCTOU — `httpx` re-resolves the host after the guard validated the IP) and **SEC-DEBT-7** (OpenAPI docs exposed in prod, which also forces `'unsafe-inline'` into the `/api` CSP). None regresses an implemented control.
...
- **SBOM & provenance:** CycloneDX SBOM per build from M0 (visibility); **signing (Sigstore/cosign) + SLSA v1.0 provenance** — **implemented** at the Hardening gate as keyless Sigstore attestation in the `containers` CI job on push to `main` (SLSA v1.0 provenance for the SBOM artifacts + SBOM attestations bound to each local image digest; job-scoped `id-token`/`attestations` perms; verify with `gh attestation verify`). Registry-image signing deferred pending a publish decision (images are built locally, not pushed). Record every bundled model's weight license in the SBOM (`ARCHITECTURE §13`).
