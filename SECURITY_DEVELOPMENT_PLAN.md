...
> - **No-fix base OS — accepted/deferred.** Remaining High/Critical are Debian base-layer CVEs with **no upstream fixed version** (web: bookworm `perl-base`, `zlib1g`, `util-linux`/`ncurses` family; api: trixie base). Nothing to bump; per policy we report base-image CVEs without a fix. Base-image supply-chain hardening (distroless/minimal base, digest-pinning `FROM` — still floating tags, TR-25) + SBOM signing/SLSA is owned by the pre-prod **Hardening gate** (§7, §10). Container-blocking stays deferred there rather than blocking on an upstream base bump or an expiring suppression. Re-verify at the Hardening gate.
