# CLAUDE.md — DAS Sentinel

> Project rules and build instructions for Claude Code (and any coding assistant) working in this repository. Read this file in full before writing code. If a request conflicts with the **Safety invariants** below, stop and surface the conflict — do not proceed.
...
| LLM | Provider abstraction (thin adapter, or LiteLLM if multi-provider); default **Anthropic Claude** (`claude-opus-4-8` workhorse, `claude-sonnet-5` for volume triage, `claude-haiku-4-5` for classification) |
| Local models | **Ollama** (dev / single-analyst / CPU) and **vLLM** (GPU-backed air-gapped server) behind the abstraction |
| First scanners | **Semgrep CE** (SAST — see license note; run against a **vendored, content-hashed, license-cleared rule bundle**, never floating registry packs), **ZAP by Checkmarx** (DAST — image **pinned by digest**, not the floating `zaproxy/zap-stable` tag). Next wave: **Nuclei ≥ v3.8.0** (content-hashed templates), OSV-Scanner v2 / pip-audit / npm audit, **Gitleaks** (default) + TruffleHog (verification) |
...
- **Semgrep license & rule pinning:** Semgrep CE engine is LGPL-2.1, but Semgrep-maintained *rules* are under the restrictive "Semgrep Rules License v1.0" (no redistribution in a competing/SaaS product) and interprocedural taint analysis is paid-only. Do **not** invoke floating registry aliases (`p/owasp-top-ten`, `p/default`) at run time — they are non-reproducible, air-gap-hostile, and pull rules under that restrictive license. Instead **vendor a content-hashed rule bundle** (pin by SHA-256, record source + license per run and in the SBOM); if we need to bundle rules or want free cross-file analysis, use **OpenGrep** (LGPL fork, rule-format compatible) rules.
- **Scanner image & signature pinning:** pin every scanner container image by **digest** (`…@sha256:<digest>`), never a mutable tag (`zaproxy/zap-stable`, `:latest`), and pin rule/signature/template bundles (Semgrep rules, Nuclei templates) by content hash. Scanner control secrets (e.g., the ZAP API key) are injected at launch and never persisted into stored scanner config.
...
- **LLM param currency:** current Claude models use `thinking: {type: "adaptive"}` only — legacy `budget_tokens`/`temperature`/`top_p` and date-suffixed IDs return HTTP 400. Prefer `claude-opus-4-8`/`claude-sonnet-5` as defaults; **Fable 5 runs cyber-content classifiers that can return `stop_reason: "refusal"` on security-adjacent prompts** — a real false-positive risk for a pentest tool, so avoid it as the default (or ship the `fallbacks` param).
