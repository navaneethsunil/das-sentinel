...
- **TR-26.4 SBOM & provenance** — a **CycloneDX SBOM** (Syft) is generated per build from M0 for visibility; **artifact signing (Sigstore/cosign) + SLSA v1.0 provenance** (OWASP A03:2025). **Implemented (Hardening):** keyless Sigstore attestation in CI on push to `main` — SLSA v1.0 build provenance for the SBOM artifacts + SBOM attestations bound to each image digest (`actions/attest-build-provenance` / `attest-sbom`); verify via `gh attestation verify … --repo`. Registry-published-image signing is deferred pending a distribution decision (CI builds images locally, not pushed). Bundled model-weight licenses are recorded in the SBOM.
