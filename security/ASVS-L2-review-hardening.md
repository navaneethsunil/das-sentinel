...
- Carried from M1: ~~**SEC-DEBT-2** (MFA)~~ **RESOLVED** — TOTP second factor + single-use recovery codes on the opaque-session login (`core/mfa.py`, `api/auth.py` enroll/confirm/disable, `api/users.py` admin reset). Secret Fernet-encrypted at rest, recovery codes SHA-256-hashed + atomically consumed, failed codes rate-limited (SEC-DEBT-1) + audited. Live-verified `scripts/verify_mfa.py` (25/25), `tests/test_mfa.py`, migration `f1a2c3d4e5f6`. ~~**SEC-DEBT-4** (append-only DB-role separation)~~ **RESOLVED** — least-privilege `das_app` runtime role (migration `a2b4c6d8e0f2`): full DML on mutable tables, SELECT/INSERT only on the 7 append-only tables — a privilege floor beneath the immutability triggers. Migrations run as the owner (`owner_database_url`); runtime switches to the restricted role when `POSTGRES_USE_APP_ROLE=true`. Live-verified `scripts/verify_db_role.py` (25/25). ~~**SEC-DEBT-5** (log retention/NTP)~~ **RESOLVED (code part)** — structured JSON stdout logs + off-box immutable audit archival to the WORM store (`app/services/audit_archive.py`, `scripts/archive_audit_log.py`, `AUDIT_ARCHIVE_RETENTION_DAYS`); off-box shipping + retention value + NTP are deployment (`security/log-retention-runbook.md`). ~~**SEC-DEBT-3** (breached-password check)~~ **RESOLVED** — offline corpus rejection at user create/change (`app/core/password_policy.py`, bundled starter list + `BREACHED_PASSWORD_LIST_PATH` override; no HIBP egress). Live-verified `scripts/verify_password_breach.py` (6/6). **No carried-M1 SEC-DEBT code items remain.**
